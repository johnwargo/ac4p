...
sap.m.Input.prototype._onsaparrowkey = function(oEvent, sDir) {
	...
	if (iSelectedIndex === -1) {
		iSelectedIndex = 0;
		if (aListItems[iSelectedIndex].getVisible() && aListItems[iSelectedIndex].getType() !== sap.m.ListType.Inactive) {
	...
	if (sDir === "down") {
		while (iSelectedIndex < aListItems.length - 1 &&
			(!bFirst || aListItems[iSelectedIndex].getVisible() === false || aListItems[iSelectedIndex].getType() === sap.m.ListType.Inactive)) {
			aListItems[iSelectedIndex].setSelected(false);
			iSelectedIndex = iSelectedIndex + 1;
			bFirst = true;
		}
	} else {
		while (iSelectedIndex > 0 && 
			(!bFirst || aListItems[iSelectedIndex].getVisible() === false || aListItems[iSelectedIndex].getType() === sap.m.ListType.Inactive)) {
			aListItems[iSelectedIndex].setSelected(false);
			iSelectedIndex = iSelectedIndex - 1;
			bFirst = true;
